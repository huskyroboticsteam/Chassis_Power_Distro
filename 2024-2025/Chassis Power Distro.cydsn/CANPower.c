...
{
    return packetToAssemble->data[4];
}
uint16_t GetOverCurrentLimitFromPacket(CANPacket *packetToAssemble)
{
    return (uint16_t)DecodeBytesToIntMSBFirst(packetToAssemble->data, 5, 6);
}